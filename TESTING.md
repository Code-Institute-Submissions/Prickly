...
- `TypeError: '<'` not supported between instances of `'datetime.datetime'` and `'NoneType'`
    - Clean method for the Product model was trying to compare added date and release date before saving them to make sure that the release date is not in the past as the item release date cannot be backdated. The release date is entered by the user, however, the added date is auto-generated by Django and at the time of the comparison, it hadn't been yet causing there to be a mismatch in types of data. To fix this I refactored the code to compare the release date which is with the current date which exposed another error as release time will always be less than now time and I refactored it again to compare days instead of DateTime which solved the issue.
    
- After migrating my Product model to Heroku, I got a `DataError` from /admin/products/product/add/
    - Everything was working as expected locally so this error took a bit of digging to understand. It turned out that the error was caused by `UUID` that is 32 characters and so in the Model, it had `max_length=32`. However, Heroku added 4 spaces in between causing the new length to be 36. This was corrected by changing the max_length and migrating data.
...
- `fatal: The remote end hung up unexpectedly error`
    - My forms wouldn't submit or validate even though I entered all data correctly and didn't see any weird feedback in my terminal.
    I tried to push my code up the Github to see if the issue remained in Deployment and received the fatal error. When looking into this I came across [this Article from stock overflow](https://stackoverflow.com/questions/6842687/the-remote-end-hung-up-unexpectedly-while-git-cloning) explaining that I needed to increase the `http.postBuffer size`. I rand `git config http.postBuffer 524288000` in my terminal and it solved the issue.
